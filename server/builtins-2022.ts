 ...
 * Extracted from official Appeon PowerScript Reference (PB 2022).
 *
 * https://docs.appeon.com/pb2022/powerscript_reference/
 */

import { FunctionInfo, ParamInfo, formatSignature, formatHover } from './builtins';

// Re-export common formatting functions for use by both versions
export { formatSignature, formatHover };

// PB 2022 functions — this will be populated from deep-research findings
// For now, starting with core functions that are stable across versions

export const builtinFunctions2022: FunctionInfo[] = [
  // String functions
  {
    name: 'Len',
    returnType: 'long',
...
const builtinIndex2022 = new Map<string, FunctionInfo>();
for (const fn of builtinFunctions2022) {
  builtinIndex2022.set(fn.name.toLowerCase(), fn);
}
